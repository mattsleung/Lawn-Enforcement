...
The public build supports email-confirmed Supabase accounts. Usernames are public, unique without regard to letter casing, and used by player search and the Trading Post. Emails remain private, and passwords are handled by Supabase Auth and never enter the repository or saved game JSON. Player saves are stored in `game_saves` with Row Level Security limiting every request to the signed-in user's own row.
